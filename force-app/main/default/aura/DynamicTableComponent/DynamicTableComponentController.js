({
    ...
    onPicklistChange : function (component, event, helper){
        ...
        if ( !$A.util.isUndefined(sel)){
            var fFields ;
             var action=component.get("c.getFilterFields");
        action.setParams({"objectName":sel});
        action.setCallback(this,function(response){
            
            var state =  response.getState();
            var  filterFields = response.getReturnValue();
            if(state == 'SUCCESS') {
                 if ( !$A.util.isEmpty(filterFields) ){
                 fFields = filterFields.split(',');
                component.set("v.filterFields" , fFields);
                 }
                else{
                     component.set("v.filterFields" , fFields);
                }
    ...
    setFilterValues   : function(component, event, helper) {
        console.log('a');
                //   var selectedItem = event.currentTarget.dataset.recid; 
